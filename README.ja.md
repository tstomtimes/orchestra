...
Claude Codeの設定でこのプラグインを追加します：

**方法A: Claude Code UIを使用**
1. Claude Code設定を開く
2. 「Plugins」セクションに移動
3. 「Add Plugin」をクリック
4. `orchestra-plugin` ディレクトリを選択
...
**セキュリティ・UI：**
- **Iris** 🛡️ _"セキュリティは最優先"_ - セキュリティのプロ。脆弱性を見逃さない鋭い目を持つ守護者
...
- **Blake** 🚀 _"準備完了、リリースしましょう"_ - リリースの指揮者。安全で確実なデプロイを保証する
...
```
orchestra-plugin/
├── agents/           # AIエージェント（Alex、Eden、Iris、Mina、Theo）
├── skills/           # 再利用可能な機能
├── hooks/            # 品質ゲートスクリプト
├── mcp-servers/      # サービス連携
...
[.claude/commands/](.claude/commands/) で利用可能なコマンド：
